...
Given an appropriate Perl and L<App-cpanminus>, you can set up a development
environment like so:
...
Your changes should pass both the tests (C<t/*>) and the extended tests
(C<xt/*>).  Dzil can run them all for you:
